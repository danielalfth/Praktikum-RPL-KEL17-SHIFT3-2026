...
<div class="page-header">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            ...
            <p>{{ $medicalRecord->created_at->translatedFormat('d F Y, H:i') }}</p>
        </div>
        <div>
            @if(auth()->user()->role === 'doctor' && $medicalRecord->doctor_id === auth()->id())
                <a href="{{ route('doctor.medical-records.edit', $medicalRecord->id) }}" class="btn btn-outline-warning btn-sm"><i class="bi bi-pencil me-1"></i> Edit</a>
            @endif
            <a href="{{ url()->previous() }}" class="btn btn-glass btn-sm"><i class="bi bi-arrow-left me-1"></i> Kembali</a>
        </div>
    </div>
</div>
